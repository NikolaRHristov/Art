	...
	<body>
		...
		<script>
			...
			const createPamphletPart = (position, rotation) => {
				const geometry = new THREE.BoxGeometry(3, 3, 0.01); // Increased width, decreased thickness
			...
			const leftPart = createPamphletPart(
				{ x: -1.5, y: 0, z: 0 },
				{ x: 0, y: Math.PI / 6, z: 0 },
			);
			const middlePart = createPamphletPart(
				{ x: 0, y: 0, z: 0 },
				{ x: 0, y: 0, z: 0 },
			);
			const rightPart = createPamphletPart(
				{ x: 1.5, y: 0, z: 0 },
				{ x: 0, y: -Math.PI / 6, z: 0 },
			);
			...
			window.addEventListener("click", (event) => {
				...
				if (intersects.length > 0) {
					...
					if (intersectedObject === leftPart) {
						leftPart.rotation.y = Math.PI / 2; // Fold completely
					}

					if (intersectedObject === rightPart) {
						rightPart.rotation.y = -Math.PI / 2; // Fold completely
					}
				}
			});
		</script>
