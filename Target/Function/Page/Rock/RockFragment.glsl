...
vec2 parallaxOcclusionMapping(
	...
	mat3 tbn_from_tangent_to_world_mat,
	...
	for(int i = 0; i <= numSteps; ++i) {
		vec3 world_offset_on_tangent_plane = (tbn_from_tangent_to_world_mat[0] * current_uv_offset.x +
			tbn_from_tangent_to_world_mat[1] * current_uv_offset.y) * uPOM_WorldSpaceUVScale;
		...
		if(uUseBakedPOMHeight) {
			height_at_current_sample = texture2D(uBakedPOMHeightMap, base_uv + current_uv_offset).r;
		...
		if(height_at_current_sample > currentLayerHeight) {
			...
			vec3 prev_world_offset_on_tangent_plane = (tbn_from_tangent_to_world_mat[0] * prev_uv_offset.x +
				tbn_from_tangent_to_world_mat[1] * prev_uv_offset.y) * uPOM_WorldSpaceUVScale;
			...
			if(uUseBakedPOMHeight) {
				height_at_prev_sample = texture2D(uBakedPOMHeightMap, base_uv + prev_uv_offset).r;
			...
			float weight = (currentLayerHeight - height_at_current_sample) /
				((height_at_prev_sample - prevLayerHeight) - (height_at_current_sample - currentLayerHeight) + 0.0001);
...
void main() {
	...
	float revealVal = fbm(vWorldPosition * uReveal_Scale, uReveal_Octaves, uReveal_Persistence, uReveal_Lacunarity, 1.0, uReveal_SeedOffset, uReveal_Amplitude);
	...
	if(uUseBakedAlbedo) {
		albedo = texture2D(uBakedAlbedoMap, final_uv_for_baked_maps).rgb;
		...
		vec3 vein_sample_pos = samplePos_world_for_3D_noise + vein_warp_noise3D * 0.05;
	...
	if(uUseBakedNormal) {
		vec3 normal_from_map = texture2D(uBakedNormalMap, final_uv_for_baked_maps).rgb * 2.0 - 1.0;

		shadingNormal = normalize(tbn_tangent_to_world * normal_from_map);

	} else
		{
	float procedural_height_for_normal(vec3 pos_world_sample) {
		return sampleTriPlanarFBM(pos_world_sample, N_world_geom_frag, uTriPlanar_BlendSharpness, uNormalDetail_Octaves, uNormalDetail_Persistence, uNormalDetail_Lacunarity, uNormalDetail_Scale, uNormalDetail_SeedOffset, uNormalDetail_Amplitude);

	}

	float delta_norm = 0.001 / max(0.0001, uNormalDetail_Scale * uPOM_WorldSpaceUVScale);

	float h_center = procedural_height_for_normal(samplePos_world_for_3D_noise);

	float h_px = procedural_height_for_normal(samplePos_world_for_3D_noise + T_world_frag * delta_norm);

	float h_py = procedural_height_for_normal(samplePos_world_for_3D_noise + B_world_frag * delta_norm);
...
roughness = texture2D(uBakedRoughnessMap, final_uv_for_baked_maps).r;
...
if(uUseBakedAO) {
ao = texture2D(uBakedAOMap, final_uv_for_baked_maps).r;
...
float roughness_pbr = roughness;

float alpha = roughness_pbr * roughness_pbr;

float alphaSq = alpha * alpha;

float D_denom = NdotH * NdotH * (alphaSq - 1.0) + 1.0;

float D_ggx = alphaSq / (PI * D_denom * D_denom + 0.00001);

float k_direct = (roughness_pbr + 1.0) * (roughness_pbr + 1.0) / 8.0;

float G_V = NdotV / (NdotV * (1.0 - k_direct) + k_direct + 0.00001);

float G_L = NdotL / (NdotL * (1.0 - k_direct) + k_direct + 0.00001);

float G_smith = G_V * G_L;

vec3 specular_brdf = D_ggx * G_smith * F / (4.0 * NdotV * NdotL + 0.0001);

vec3 kD = (vec3(1.0) - F);

vec3 diffuse_brdf = kD * albedo / PI;

vec3 outgoingLight = (diffuse_brdf + specular_brdf) * uDirLightColor * NdotL;

vec3 ambientLight = uAmbientLightColor * albedo * ao;

vec3 finalColorLinear = ambientLight + outgoingLight;

finalColorLinear = finalColorLinear * 0.6;

vec3 x = finalColorLinear;

x = (x * (x * 2.51 + 0.03)) / (x * (x * 2.43 + 0.59) + 0.14);

finalColorLinear = pow(x, vec3(1.0 / 2.2));

gl_FragColor = vec4(finalColorLinear, 1.0);
